...
def rotear_para_agente_apropriado(
    ...
    Versão otimizada com detecção multi-agente melhorada e keywords expandidas.
    """
    ...
    keywords_redator = [
        ...
        # NOVOS: termos específicos de escrita acadêmica
        'estruturar', 'estrutura de', 'redigir', 'redação', 'redija',
        'parágrafo', 'parágrafos', 'capítulo', 'capítulos', 'seção', 'seções',
        'orientação acadêmica', 'orientar', 'como escrever',
        'texto acadêmico', 'desenvolvimento', 'argumentação',
        'coerência', 'coesão', 'normas', 'citações'
    ]
    ...
    # PRIORIDADE 1: Formatação de referências → Dr. Redator
    # Deve vir ANTES de contar matches para evitar conflito com keywords de ciencia
    formatting_keywords = ['abnt', 'vancouver', 'apa', 'formate', 'formatação', 'formatar']
    reference_keywords = ['referências', 'citações', 'bibliografia']
    ...
    if has_formatting and has_references:
        # logger.info("Roteamento (prioridade formatação)", ...)
        return 'redator'
    ...
    # MELHORIA: Detectar triggers de coordenação explícita
    # Strings simples para busca rápida
    simple_triggers = [
        # Coordenação com "e"
        'e também', 'e depois', 'e criar', 'e escrever', 'e gerar',
        'e formate', 'e formatar',
        # Base/Dependência
        'com base em', 'baseado em', 'baseado na', 'usando',
        # Sequência temporal
        'primeiro', 'depois', 'e então', 'em seguida',
    ]
    
    # Padrões regex para detecção mais sofisticada
    regex_patterns = [
        r'pesquis\w+.*e',  # pesquisar/pesquise... e
        r'busc\w+.*e',     # buscar/busque... e
        r'encontr\w+.*e',  # encontrar/encontre... e
        r'escrev\w+.*com', # escrever/escreva... com
        r'escrev\w+.*usando' # escrever/escreva... usando
    ]
    
    has_coordination = (
        any(trigger in mensagem_lower for trigger in simple_triggers) or
        any(re.search(pattern, mensagem_lower) for pattern in regex_patterns)
    )
    
    # MELHORIA: Detectar se há keywords de DIFERENTES domínios (mesmo que 1 de cada)
    # Isso captura casos como "pesquise artigos e crie questões"
    domains_matched = sum([
        matches_ciencia >= 1,
        matches_estudo >= 1,
        matches_redator >= 1
    ...
    # Se tem coordenação E múltiplos domínios, usar equipe
    if has_coordination and domains_matched >= 2:
        return 'equipe'
    
    # FALLBACK: Se múltiplos agentes têm match alto (2+), usar equipe
    high_matches = sum([
        matches_ciencia >= 2,
        matches_estudo >= 2,
        matches_redator >= 2
...
class RoutingTester:
    ...
    def run_tests(self, verbose: bool = True):
        ...
        for i, test in enumerate(self.test_cases, 1):
            resultado = rotear_para_agente_apropriado(
                mensagem_usuario=test.mensagem,
                tem_imagem=test.tem_imagem
            )
        ...
        if failed > 0:
            ...
            for result in self.results:
                if not result['sucesso']:
                    ...
                    print(f"    Esperado '{result['esperado']}' mas obteve '{result['obtido']}'")
            print()
    ...
    def analyze_routing_logic(self):
        ...
        for cat, stats in categorias.items():
            ...
            print(f"  {cat}:")
            print(f"    Total: {stats['total']}, Sucesso: {stats['sucesso']}, "
...
def main():
    ...
    if taxa >= 90:
        ...
        print("⚠ Sistema de roteamento está funcionando BEM, mas há espaço para melhorias.")
        print(f"  Taxa de sucesso: {taxa:.1f}%")
    else:
