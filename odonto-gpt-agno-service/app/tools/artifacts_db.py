...
def _get_supabase_client() -> Client:
    ...
    key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY") # Use Service Role para garantir permissões de escrita em nome do user
...
def save_research(
    ...
    This should be called when the user asks to save a research, or typically automatically after generating a comprehensive answer using research tools.

    Args:
        user_id (str): The ID of the user (provided in context)
        title (str): Title of the research
        content (str): The full markdown content of the research
        query (str): The search query used
        sources (List[Dict]): List of sources [{"title": "...", "url": "..."}]
        research_type (str): Type of research (literature_review, clinical_trial_summary)

    Returns:
        str: Confirmation message with Artifact ID
    """
    try:
